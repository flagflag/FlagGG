...
int main(int argc, char** argv)
{
	...
	string* bindingTag = (argc >= 4) ? new string(argv[3]) : NULL;
    string* includeAfx = (argc >= 5) ? new string(argv[4]) : NULL;
	string* bindingNS = (argc >= 6) ? new string(argv[5]) : NULL;
    Generator::getInstance()->run(argv[1], (argc >= 3) ? argv[2] : argv[1], bindingTag, includeAfx, bindingNS);
    Generator::releaseInstance();
	SAFE_DELETE(bindingTag);
