...
public:
     ...
     */
    void run(string inDir, string outDir, string* bindingTag= NULL, string* includeAfx= NULL, string* bindingNS = NULL);

    bool isDataBlock(string classname);
	bool isReferenced(string classname);

	bool isClassBindingFromRef(string refId);

	void writeEnum(ostream& o, EnumBinding& binding, int indentLevel);
	void writeEnum(ostream& o, string enumName, int indentLevel);
...
protected:
    ...
    void getEnums(XMLElement* fileNode, string ns = "", string include = "");
    
    // Parses the enumeration type and adds it to the global list.
    void getEnum(XMLElement* e, ClassBinding* classbinding, string ns = "", string include = "");

    // Parses all typedefs within a file (optionall within a namespace).
    void getTypedefs(XMLElement* fileNode, string ns = "", string include = "");

    // Parses the typedef and adds it to the global list.
    void getTypedef(XMLElement* e, string classname = "", string ns = "");

	void getStructs(XMLElement* fileNode, string ns = "", string include = "");
	void getStruct(XMLElement* e, string include = "", string ns = "");

    // Gets the script flag if it is specified.
    string getScriptFlag(XMLElement* e);

	string getIncludeFlag(XMLElement* e);

	bool isExport(XMLElement* e);
