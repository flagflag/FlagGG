...
static void DecodePackage(const String& packagePath, const String& decodePackagePath)
{
	const String decodeExe = "E:/GitProject/NE/Urho/vs/bin/SSWCommandlet.exe";

	Vector<String> results;
	auto* localFileSystem = GetLocalFileSystem();
	localFileSystem->FindFiles(results, packagePath, "*.*", true, FindFileMode::FILE);

	for (auto& relativePath : results)
	{
		const String inputPath = packagePath + "/" + relativePath;
		const String outputPath = decodePackagePath + "/" + relativePath;
		const String outputDirectory = GetPath(outputPath);

		if (!localFileSystem->DirectoryExists(outputDirectory))
		{
			localFileSystem->CreateDirectory(outputDirectory);
		}

		const String command = decodeExe + " UnConfuseFile in=" + inputPath + " out=" + outputPath;
...
void Run()
{
	...
	DecodePackage("E:/GitProject/NE/Res/characters1/jianke_c96b", "E:/GitProject/NE/Res/characters1/jianke");
}
