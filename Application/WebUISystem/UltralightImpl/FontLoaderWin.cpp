...
String UltralightFontLoader::fallback_font() const
{
	return "Arial";
}

String UltralightFontLoader::fallback_font_for_characters(const String& characters, int weight, bool italic) const
{
...
RefPtr<FontFile> UltralightFontLoader::Load(const String& family, int weight, bool italic)
{
	...
	const String16 family16 = family.utf16();

	UINT32 index = 0;
	BOOL exists = false;
	size_t len = family16.length() + 1;
	WCHAR* family_name_wcstr = new WCHAR[len];
	memset(family_name_wcstr, 0, sizeof(WCHAR) * len);
	memcpy(family_name_wcstr, family16.data(), sizeof(WCHAR) * (len - 1));
...
RefPtr<FontFile> UltralightFontLoader::Load(const String& family, int weight, bool italic)
{
	...
	auto ultralightBuffer = Buffer::CreateFromCopy(fileContent.CString(), fileContent.Length());

	return FontFile::Create(ultralightBuffer);
}
