...
static void WriteVertex(IOFrame::Buffer::StringBuffer& buffer, aiMesh* aiMsh, bool isSkinned, UInt32 index, 
...
static void WriteIndex(IOFrame::Buffer::StringBuffer& buffer, aiMesh* aiMsh, UInt32 index, bool largeIndeces)
{
...
static SharedPtr<Node> BuildNode(const aiScene* aiScn, aiNode* aiNd, aiMatrix4x4 transform = {})
{
	...
	if (aiNd->mNumMeshes > 0)
	{
		...
		for (UInt32 i = 0; i < aiNd->mNumMeshes; ++i)
		{
			...
			IOFrame::Buffer::StringBuffer buffer1(vertexBuffer->Lock(0, vertexBufferSize), vertexBufferSize);
			...
			IOFrame::Buffer::StringBuffer buffer2(indexBuffer->Lock(0, indexBufferSize), indexBufferSize);
