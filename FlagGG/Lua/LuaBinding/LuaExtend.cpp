...
namespace FlagGG
{
	namespace Lua
	{
		...
		bool luaex_juedge_inherit(lua_State* L, const Container::String& type, const Container::String& superClass)
		{
			// 拿出继承关系
			luaex_pushcontext(L);
			...
			Container::String subClass = type;
			bool findIt = false;
		...
		int luaex_pushusertyperef(lua_State* L, const char* userType, Container::RefCounted* ref)
		{
		...
		void* luaex_tousertype(lua_State* L, int idx, const char* userType)
		{
			...
			Container::String type = lua_tostring(L, -1);
			lua_pop(L, 1);
