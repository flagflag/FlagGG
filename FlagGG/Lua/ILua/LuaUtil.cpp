...
bool GetGameEvent(lua_State* L, const String& eventName)
{
	static String nameSpaceOrName, funcName;

	auto nPos = eventName.Find(".");
	nameSpaceOrName = nPos != String::NPOS ? eventName.Substring(0, nPos) : eventName;
	...
	if (TypeOf(L, -1) != Type::FUNCTION)
	{
		...
		String content = "try call non-function[" + eventName + "].";
