...
void LuaVM::RegisterCEvents(const Vector<LuaProxy>& librarys)
{
...
void LuaVM::RegisterCPPEvents(const String& className, void* instance, const Vector<LuaProxy>& librarys)
{
	lua_newtable(luaState_);
	SharedArrayPtr<luaL_Reg> libs(new luaL_Reg[librarys.Size() + 1]);
	for (UInt32 i = 0; i < librarys.Size(); ++i)
	{
...
bool LuaVM::Execute(const String& filePath)
{
...
bool LuaVM::ExecuteScript(const String& scriptCode)
{
...
void LuaVM::SetLoaderPath(const String& loaderPath)
{
	String modePath = loaderPath + "?.lua";
