...
public:
	...
	Real AbsDotProduct(const Vector2& rhs) const { return FlagGG::Abs(x_ * rhs.x_) + FlagGG::Abs(y_ * rhs.y_); }

	Real ProjectOntoAxis(const Vector2& axis) const { return DotProduct(axis.Normalized()); }

	Real Angle(const Vector2& rhs) const { return FlagGG::Acos(DotProduct(rhs) / (Length() * rhs.Length())); }
