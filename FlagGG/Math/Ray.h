...
public:
    ...
    float HitDistance(const BoundingBox& box, float insideValue = 0.0f) const;
    ...
    float HitDistance(const Vector3& v0, const Vector3& v1, const Vector3& v2, Vector3* outNormal = nullptr, Vector3* outBary = nullptr, bool doubleSide = false) const;
    ...
    /// |                       |
    /// -------------------------
    bool InsideMultiGeometry(const void* vertexData, unsigned vertexSize, const void* indexData, unsigned indexSize, unsigned indexStart,
        unsigned indexCount) const;
