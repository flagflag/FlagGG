...
public:
	...
	// UE3专用，因为UE3的四元数跟引擎的不一样，引擎用欧拉角的话会有万向锁
	Matrix3x4(const Vector3& translation, const Vector3& rotation, const Vector3& scale) noexcept
	{
		float roll = rotation.x_;
		float pitch = rotation.y_;
		float yaw = rotation.z_;
		...
		m01_ = (SR * SP * CY - CR * SY)  * scale.y_;
		m11_ = (SR * SP * SY + CR * CY)  * scale.y_;
		m21_ = (-SR * CP) * scale.y_;

		m02_ = (-(CR * SP * CY + SR * SY)) * scale.z_;
		m12_ = (CY * SR - CR * SP * SY)  * scale.z_;
		m22_ = (CR * CP) * scale.z_;
	...
	/// Add a number
	Matrix3x4 operator +(float value) const
	{
		return Matrix3x4(
			m00_ + value,
			m01_ + value,
	...
	float operator[](unsigned index) const { return (&m00_)[index]; }
    ...
    /// Return the positively oriented rotation matrix with scaling removed.
    Matrix3 PositiveRotationMatrix() const
    {
        Vector3 F = Column(0);  // Forward
        Vector3 R = Column(1);  // Right
        Vector3 U = Column(2);  // Up
        float det = F.DotProduct(R.CrossProduct(U)); // determinant
        Vector3 invScale(
            1.0f / sqrtf(m00_ * m00_ + m10_ * m10_ + m20_ * m20_),
            1.0f / sqrtf(m01_ * m01_ + m11_ * m11_ + m21_ * m21_),
            1.0f / sqrtf(m02_ * m02_ + m12_ * m12_ + m22_ * m22_)
        ...
        F *= invScale.x_;
        R *= det > 0 ? invScale.y_ : -invScale.y_;
        U *= invScale.z_;
    ...
    Quaternion Rotation() const { return Quaternion(PositiveRotationMatrix()); }
    ...
    /// Return the scaling part with the sign. Reference rotation is Rotation().
    Vector3 SignedScale() const
    {
        Vector3 F = Column(0);  // Forward
        Vector3 R = Column(1);  // Right
        Vector3 U = Column(2);  // Up
        ...
        return Vector3(
            sqrtf(m00_ * m00_ + m10_ * m10_ + m20_ * m20_),
            sqrtf(m01_ * m01_ + m11_ * m11_ + m21_ * m21_) * det > 0 ? 1.f : -1.f,
            sqrtf(m02_ * m02_ + m12_ * m12_ + m22_ * m22_)
        );
    }
    ...
    bool Equals(const Matrix3x4& rhs) const
    {
        ...
        for (unsigned i = 0; i < 12; ++i)
        {
            if (!FlagGG::Equals(leftData[i], rightData[i]))
                return false;
        }
