...
public:
...
#if defined(FLAGGG_SSE) && (!defined(_MSC_VER) || _MSC_VER >= 1700) /* Visual Studio 2012 and newer. VS2010 has a bug with these, see https://github.com/urho3d/Urho3D/issues/1044 */
    {
	...
	{
		FromEulerAngles(euler.x_, euler.y_, euler.z_);
	}
    ...
    /// Test for equality with another quaternion with custom tolerance.
    bool Equals(const Quaternion& rhs, float tolerance) const
    {
        return FlagGG::Equals(w_, rhs.w_, tolerance) && FlagGG::Equals(x_, rhs.x_, tolerance) && FlagGG::Equals(y_, rhs.y_, tolerance) && FlagGG::Equals(z_, rhs.z_, tolerance);
    }

    /// Return whether is NaN.
    bool IsNaN() const { return FlagGG::IsNaN(w_) || FlagGG::IsNaN(x_) || FlagGG::IsNaN(y_) || FlagGG::IsNaN(z_); }
