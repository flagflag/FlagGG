...
void ProjectAndMergeEdge(Vector3 v0, Vector3 v1, Rect& rect, const Matrix4& projection)
{
    ...
    if (v0.z_ < F_MIN_NEARCLIP && v1.z_ < F_MIN_NEARCLIP)
        return;

    // Check if need to clip one of the vertices
    if (v1.z_ < F_MIN_NEARCLIP)
        v1 = ClipEdgeZ(v1, v0, F_MIN_NEARCLIP);
    else if (v0.z_ < F_MIN_NEARCLIP)
        v0 = ClipEdgeZ(v0, v1, F_MIN_NEARCLIP);
...
void Frustum::Define(float fov, float aspectRatio, float zoom, float nearZ, float farZ, const Matrix3x4& transform)
{
    ...
    farZ = Max(farZ, nearZ);
    float halfViewSize = tanf(fov * F_DEGTORAD_2) / zoom;
    Vector3 near, far;
