...
namespace FlagGG
{
	namespace Core
	{
		Profiler::Profiler() :
			...
			intervalFrames_(0)
		{
			Utility::SystemHelper::HiresTimer::InitSupported();
		...
		const Container::String& Profiler::PrintData(bool showUnused, bool showTotal, unsigned maxDepth) const
		{
			static Container::String output;
		...
		void Profiler::PrintData(ProfilerBlock* block, Container::String& output, unsigned depth, unsigned maxDepth, bool showUnused,
			bool showTotal) const
		{
			...
			if (showUnused || block->intervalCount_ || (showTotal && block->totalCount_))
			{
				...
				if (!showTotal)
				{
					...
					sprintf(line, "%s %5u %8.3f %8.3f %8.3f %9.3f\n", indentedName, Math::Min(block->intervalCount_, 99999U),
						avg, max, frame, all);
				}
				else
				{
					...
					sprintf(line, "%s %5u %8.3f %8.3f %9.3f  %7u %9.3f %9.3f %11.3f\n", indentedName, Math::Min(block->frameCount_, 99999U),
						avg, max, all, Math::Min(block->totalCount_, 99999U), totalAvg, totalMax, totalAll);
				}

				output += Container::String(line);
			}

			for (Container::PODVector<ProfilerBlock*>::ConstIterator i = block->children_.Begin(); i != block->children_.End(); ++i)
