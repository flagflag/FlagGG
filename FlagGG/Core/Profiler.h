	...
	namespace Core
	{
		...
		public:
				...
				totalCount_(0)
			{
				if (name)
				{
					unsigned nameLength = Container::String::CStringLength(name);
			...
			{
				for (Container::PODVector<ProfilerBlock*>::Iterator i = children_.Begin(); i != children_.End(); ++i)
				{
					delete *i;
					*i = nullptr;
				}
			...
			ProfilerBlock* GetChild(const char* name)
			{
				for (Container::PODVector<ProfilerBlock*>::Iterator i = children_.Begin(); i != children_.End(); ++i)
				{
					if (!Container::String::Compare((*i)->name_, name, true))
						return *i;
				}
			...
			char* name_;
			// 时间统计器
			Utility::SystemHelper::HiresTimer timer_;
			// 当前帧的耗时
			long long time_;
			// 当前帧为止最大耗时
			long long maxTime_;
			...
			// Profiler块孩子节点
			Container::PODVector<ProfilerBlock*> children_;
			// 前一帧耗时
		...
		public:
			...
			const Container::String& PrintData(bool showUnused = false, bool showTotal = false, unsigned maxDepth = Math::F_MAX_UNSIGNED) const;
			const ProfilerBlock* GetCurrentBlock() { return current_; }
			const ProfilerBlock* GetRootBlock() { return root_; }

		protected:
			void PrintData(ProfilerBlock* block, Container::String& output, unsigned depth, unsigned maxDepth, bool showUnused, bool showTotal) const;
