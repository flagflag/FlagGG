...
static void *mmap_plain(PRNGState* rs, size_t size)
{
  DWORD olderr = GetLastError();
...
/* For direct MMAP, use MEM_TOP_DOWN to minimize interference */
static void *direct_mmap(PRNGState* rs, size_t size)
{
  DWORD olderr = GetLastError();
...
#define CALL_MMAP(prng, size)	mmap_plain(prng, size)
#define DIRECT_MMAP(prng, size)	direct_mmap(prng, size)
