...
UDPConnector::UDPConnector(Handler::EventHandlerPtr handler, IOFrame::IOThreadPoolPtr& thread_pool) :
	channel_(new Channel::UDPChannel(DynamicCast<IOFrame::UDPThreadPool>(thread_pool)->getService())),
	handler_(handler)
{
	DynamicCast<IOFrame::UDPThreadPool>(thread_pool)->SetPacketReciver(std::bind(&UDPConnector::PacketReciver, this, std::placeholders::_1));
