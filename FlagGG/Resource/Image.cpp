...
namespace FlagGG
{
	namespace Resource
	{
		...
		Image::Image(Core::Context* context) :
			Resource(context)
		{ }
		...
		bool Image::BeginLoad(IOFrame::Buffer::IOBuffer* stream)
		{
			...
			if (fileID == "DDS ")
			{
				...
				{
					dataSize = (ddsd.ddpfPixelFormat_.dwRGBBitCount_ / 8) * ddsd.dwWidth_ * ddsd.dwHeight_ * Math::Max(ddsd.dwDepth_, 1U);
					// Calculate mip data size
					unsigned x = ddsd.dwWidth_ / 2;
					unsigned y = ddsd.dwHeight_ / 2;
					unsigned z = ddsd.dwDepth_ / 2;
					for (unsigned level = ddsd.dwMipMapCount_; level > 1; x /= 2, y /= 2, z /= 2, --level)
						dataSize += (ddsd.ddpfPixelFormat_.dwRGBBitCount_ / 8) * Math::Max(x, 1U) * Math::Max(y, 1U) * Math::Max(z, 1U);
				}
				...
				for (unsigned faceIndex = 0; faceIndex < imageChainCount; ++faceIndex)
				{
					...
					if (faceIndex < imageChainCount - 1)
					{
						...
						Container::SharedPtr<Image> nextImage(new Image(context_));
	...
	{ \
		while (((mask) << (l)) < 0x80) \
			++(l); \
						...
						Container::SharedArrayPtr<unsigned char> rgbaData(new unsigned char[numPixels * 4]);
		...
		void Image::SetPixel(int x, int y, const Math::Color& color)
		{
			SetPixelInt(x, y, 0, color.ToUInt());
		}

		void Image::SetPixel(int x, int y, int z, const Math::Color& color)
		{
		...
		bool Image::FlipHorizontal()
		{
			...
			if (!IsCompressed())
			{
				Container::SharedArrayPtr<unsigned char> newData(new unsigned char[width_ * height_ * components_]);
			...
			else
			{
				...
				Container::SharedArrayPtr<unsigned char> newData(new unsigned char[GetMemoryUse()]);
				unsigned dataOffset = 0;
		...
		bool Image::Resize(int width, int height)
		{
			...
			Container::SharedArrayPtr<unsigned char> newData(new unsigned char[width * height * components_]);
		...
		bool Image::SaveBMP(const Container::String& fileName) const
		{
		...
		bool Image::SavePNG(const Container::String& fileName) const
		{
		...
		bool Image::SaveTGA(const Container::String& fileName) const
		{
			if (!Utility::SystemHelper::HasAccess(fileName))
			{
				FLAGGG_LOG_ERROR("Access denied to %s", fileName.CString());
			...
			const Container::String formatPath = Utility::SystemHelper::FormatPath(fileName);
			return stbi_write_tga(formatPath.CString(), width_, height_, components_, data_.Get()) != 0;
		}

		bool Image::SaveJPG(const Container::String& fileName, int quality) const
		{
			if (!Utility::SystemHelper::HasAccess(fileName))
			{
				FLAGGG_LOG_ERROR("Access denied to %s.", fileName.CString());
		...
		bool Image::SaveDDS(const Container::String& fileName) const
		{
			IOFrame::Stream::FileStream outFile;
			outFile.Open(fileName, IOFrame::Stream::FileMode::FILE_WRITE);
			if (!outFile.IsOpen())
			...
			// Write image
			Container::PODVector<const Image*> levels;
			GetLevels(levels);
		...
		Math::Color Image::GetPixel(int x, int y, int z) const
		{
		...
		Math::Color Image::GetPixelTrilinear(float x, float y, float z) const
		{
			...
			Math::Color colorNear = topColorNear.Lerp(bottomColorNear, yF);
			Math::Color topColorFar = GetPixel(xI, yI, zI + 1).Lerp(GetPixel(xI + 1, yI, zI + 1), xF);
			Math::Color bottomColorFar = GetPixel(xI, yI + 1, zI + 1).Lerp(GetPixel(xI + 1, yI + 1, zI + 1), xF);
			Math::Color colorFar = topColorFar.Lerp(bottomColorFar, yF);
			return colorNear.Lerp(colorFar, zF);
		}

		Container::SharedPtr<Image> Image::GetNextLevel() const
		{
			if (IsCompressed())
			{
				FLAGGG_LOG_ERROR("Can not generate mip level from compressed data");
				return Container::SharedPtr<Image>();
			}
			...
			Container::SharedPtr<Image> mipImage(new Image(context_));
		...
		Container::SharedPtr<Image> Image::ConvertToRGBA() const
		{
			if (IsCompressed())
			{
			...
			// Already RGBA?
			if (components_ == 4)
				return Container::SharedPtr<Image>(const_cast<Image*>(this));

			Container::SharedPtr<Image> ret(new Image(context_));
		...
		CompressedLevel Image::GetCompressedLevel(unsigned index) const
		{
			...
			else
			{
				...
				for (;;)
				{
					...
					int dataWidth = Math::Max(level.width_, level.blockSize_ == 2 ? 16 : 8);
					int dataHeight = Math::Max(level.height_, 8);
					level.data_ = data_.Get() + offset;
		...
		Image* Image::GetSubimage(const Math::IntRect& rect) const
		{
			if (!data_)
				return nullptr;
			...
			else
			{
				...
				Math::IntRect currentRect = paddedRect;

				Container::PODVector<unsigned char> subimageData;
		...
		void Image::PrecalculateLevels()
		{
			...
			{
				Container::SharedPtr<Image> current = GetNextLevel();
				nextLevel_ = current;
		...
		void Image::GetLevels(Container::PODVector<Image*>& levels)
		{
			levels.Clear();
		...
		void Image::GetLevels(Container::PODVector<const Image*>& levels) const
		{
			levels.Clear();
		...
		bool Image::SetSubimage(const Image* image, const Math::IntRect& rect)
		{
			if (!data_)
				return false;
