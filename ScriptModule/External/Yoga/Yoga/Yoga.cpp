...
#define inline __inline
 ...
 // @jonathanma fminf and fmaxf needs to check for NaN like the C implementation
 //#if _MSC_VER < 1800
#endif
...
#if defined(_N3DS)
__forceinline float roundf(const float x)
{
...
typedef struct YGNode {
  ...
  YGPrintFunc print;
  YGConfigRef config;
  // BEGIN_UNITY @jonathanma Removed Node context, we use ScriptingGCHandle instead
  //void *context;
  // END_UNITY
  ...
  // BEGIN_UNITY @jonathanma Added managed object handle for callbacks invocation
  ScriptingGCHandle managedObjectHandle;
  // END_UNITY
} YGNode;

// BEGIN_UNITY @jonathanma AggregateInit
...
static const YGNode gYGNodeDefaults = {
    // Style
    {
        ...
        YGFlexDirectionColumn, // flexDirection
        YGJustifyFlexStart, // justifyContent
        YGAlignFlexStart, // alignContent
        YGAlignStretch, // alignItems
        YGAlignAuto, // alignSelf
        YGPositionTypeRelative, // positionType
        YGWrapNoWrap, // flexWrap
        YGOverflowVisible, // overflow
        YGDisplayFlex, // display
        YGUndefined, // flex
        YGUndefined, // flexGrow
        YGUndefined, // flexShrink
        YG_AUTO_VALUES, // flexBasis
        YG_DEFAULT_EDGE_VALUES_UNIT, // margin
        YG_DEFAULT_EDGE_VALUES_UNIT, // position
        YG_DEFAULT_EDGE_VALUES_UNIT, // padding
        YG_DEFAULT_EDGE_VALUES_UNIT, // border
        YG_DEFAULT_DIMENSION_VALUES_AUTO_UNIT, // dimensions
        YG_DEFAULT_DIMENSION_VALUES_UNIT, // minDimensions
        YG_DEFAULT_DIMENSION_VALUES_UNIT, // maxDimensions
        YGUndefined, // aspectRatio
    },
    // Layout
    { 
        {}, // position
        YG_DEFAULT_DIMENSION_VALUES, // dimensions
        {}, // margins
        {}, // border
        {}, // padding
        YGDirectionInherit, // direction
        0, // computedFlexBasisGeneration
        YGUndefined, // computedFlexBasis
        false, // hadOverflow
        0, // generationCount
        (YGDirection)-1, // lastParentDirection
        0, // nextCachedMeasurementsIndex 
        {}, // cachedMeasurements
        YG_DEFAULT_DIMENSION_VALUES, // measuredDimensions 
        ...
        // cachedLayout
        {
            ...
            0.0f, // parentWidth
            0.0f, // parentHeight
            (YGMeasureMode)-1, // widthMeasureMode 
            (YGMeasureMode)-1, // heightMeasureMode 
            -1.0f, // computedWidth 
            -1.0f // computedHeight 
        }
    },
    ...
    false, // isDirty
    true, // hasNewLayout
    YGNodeTypeDefault, // nodeType
    {&YGValueUndefined, &YGValueUndefined}, // resolveDimensions
    ScriptingGCHandle() //managedObjectHandle
};
// END_UNITY
...
static YGConfig gYGConfigDefaults = {
    ...
    false, // useWebDefaults
    false, // useLegacyStretchBehaviour
    1.0f, // pointScaleFactor
#ifdef ANDROID
    &YGAndroidLog, // logger
#else
    &YGDefaultLog, // logger
#endif
    NULL, // cloneNodeCallback
    NULL, // context
};
// END_UNITY
...
// BEGIN_UNITY @jonathanma : Returns 0 if resolvedMargin is NaN
// If the resolve fail, we do not want to return NaN.
// This would cause all following calculation with that value to be NaN as wel.
// and the layout would end up being invalid.
static inline float YGResolveValueMargin(const YGValue *const value, const float parentSize) {
  if (value->unit == YGUnitAuto)
    return 0;

  const float resolvedMargin = YGResolveValue(value, parentSize);
  return YGFloatIsUndefined(resolvedMargin) ? 0 : resolvedMargin;
}
// END_UNITY

// BEGIN_UNITY @jonathaman Namespace
namespace
{
...
WIN_EXPORT YGNodeRef YGNodeNewWithConfig(const YGConfigRef config) {
  ...
  const YGNodeRef node = (YGNodeRef)gYGMalloc(sizeof(YGNode));
  // END_UNITY
...
void YGNodeFree(const YGNodeRef node) {
  ...
  // BEGIN_UNITY @jonathanma Release managed ref
  if (node->managedObjectHandle.HasTarget())
  {
      node->managedObjectHandle.ReleaseAndClear();
  }
  // END_UNITY
...
YGConfigRef YGConfigNew(void) {
  // BEGIN_UNITY @jonathanma CompileForVS2010
  const YGConfigRef config = (YGConfigRef)gYGMalloc(sizeof(YGConfig));
  // END_UNITY
...
void YGNodeRemoveChild(const YGNodeRef parent, const YGNodeRef excludedChild) {
  ...
  if (firstChild->parent == parent) {
    ...
    if (YGNodeListDelete(parent->children, excludedChild) != NULL) {
      // BEGIN_UNITY @jonathanma Do not reset layout on remove
      // This allow moving childs in the hierarchy by doing add + remove 
...
// BEGIN_UNITY @jonathanma Added managed object handle for callbacks invocation
void YGSetManagedObject(YGNodeRef node, ScriptingObjectPtr managedObject)
{
    YGAssertWithNode(node,
        managedObject == SCRIPTING_NULL || !node->managedObjectHandle.HasTarget() ||
        node->managedObjectHandle.Resolve() == managedObject,
        "Node already have a valid managed object set");

    if (node->managedObjectHandle.HasTarget()) {
        node->managedObjectHandle.ReleaseAndClear();
    }
    node->managedObjectHandle.AcquireWeak(managedObject);
}

ScriptingObjectPtr YGGetManagedObject(const YGNodeRef node)
{
    return node->managedObjectHandle.Resolve();
}
// END_UNITY
...
float YGNodeGetComputedFlexBasis(const YGNodeRef node)
{
    return node->layout.computedFlexBasis;
}
...
static inline void YGResolveDimensions(YGNodeRef node) {
  ...
  for (int dim = YGDimensionWidth; dim <= YGDimensionHeight; dim++) {
  // END_UNITY
...
static void YGPrintEdges(YGStringStream *stream, const char *str, const YGValue *edges) {
  if (YGFourValuesEqual(edges)) {
    ...
    for (int edge = YGEdgeLeft; edge < YGEdgeCount; edge++) {
      char buf[30];
...
static inline bool YGNodeIsStyleLoosePercentage(const YGNodeRef node,
                                                const YGFlexDirection mainAxis,
                                                const YGMeasureMode widthMeasureMode,
                                                const YGMeasureMode heightMeasureMode) {

  if (widthMeasureMode != YGMeasureModeExactly) {
    if (node->style.dimensions[YGDimensionWidth].unit == YGUnitPercent ||
        node->style.minDimensions[YGDimensionWidth].unit == YGUnitPercent ||
        node->style.maxDimensions[YGDimensionWidth].unit == YGUnitPercent ||
        node->style.margin[YGEdgeLeft].unit == YGUnitPercent ||
        node->style.margin[YGEdgeTop].unit == YGUnitPercent ||
        node->style.margin[YGEdgeRight].unit == YGUnitPercent ||
        node->style.margin[YGEdgeBottom].unit == YGUnitPercent ||
        node->style.padding[YGEdgeLeft].unit == YGUnitPercent ||
        node->style.padding[YGEdgeTop].unit == YGUnitPercent ||
        node->style.padding[YGEdgeRight].unit == YGUnitPercent ||
        node->style.padding[YGEdgeBottom].unit == YGUnitPercent) {
      return true;
    }
  }

  if (heightMeasureMode != YGMeasureModeExactly) {
    if (node->style.dimensions[YGDimensionHeight].unit == YGUnitPercent ||
  ...
  // flex-basis 0% is used by the flex shorthand and has a different meaning then flex-basis 0.
  // The difference is that 0% is indefinite and will trigger a measurement of the node.
  // O% doesn't require a post layout because it is still 0, so we just return false in that case!
  if (node->style.flexBasis.unit == YGUnitPercent && !YGFloatsEqual(node->style.flexBasis.value, 0)) {
    const bool isMainAxisRow = YGFlexDirectionIsRow(mainAxis);
    return isMainAxisRow ? widthMeasureMode != YGMeasureModeExactly : heightMeasureMode != YGMeasureModeExactly;
  }
...
static bool YGNodeSetPosition(const YGNodeRef node,
                              const YGDirection direction,
                              const YGFlexDirection mainAxis,
  ...
  // If a relative position is unknown we need to do another layout pass
  // This can happen when positions are using percentage and the parent size is unknown in that axis
  if (YGFloatIsUndefined(relativePositionMain) || YGFloatIsUndefined(relativePositionCross))
    return false;
...
static void YGNodeComputeFlexBasisForChild(const YGNodeRef node,
  ...
  // BEGIN_UNITY @jonathanma : percent requires to recompute the flex basis because parent dim may have changed
  // Commented out !YGFloatIsUndefined(mainAxisSize) because if the node is absolute mainAxisSize will be undefined
  // and if the child has a resolvedFlexBasis it will be totally ignore.
  // When flex: 1 is use the flex-basis will be 0% and resolvedFlexBasis undefined, this cause a measurement
  // of the node below. This is compliant to the browsers implementation.
...
static void YGNodelayoutImpl(const YGNodeRef node,
  ...
  // Make sure that children of this node do not use parentWidth/parentHeight for percentage
  // resolution. This is the case on vanilla Yoga...
  // nodeWidth and nodeHeight are what MUST be pass to YGResolveValue.
  // If they are unknown there will be another layout pass once they are known (only if a children depends on it).
  // See requirePostLayout boolean below.
  const bool isExactMeasurements = widthMeasureMode == YGMeasureModeExactly && heightMeasureMode == YGMeasureModeExactly;
  const float nodeWidth = widthMeasureMode == YGMeasureModeExactly ? availableInnerWidth : YGUndefined;
  const float nodeHeight = heightMeasureMode == YGMeasureModeExactly ? availableInnerHeight : YGUndefined;

  const float mainAxisNodeSize = isMainAxisRow ? nodeWidth : nodeHeight;
  const float crossAxisNodeSize = isMainAxisRow ? nodeHeight : nodeWidth;
  const float mainAxisParentSize = isMainAxisRow ? parentWidth : parentHeight;
  ...
  for (uint32_t i = 0; i < childCount; i++) {
    ...
    if (performLayout) {
      ...
      const bool positionIsSet = YGNodeSetPosition(child,
                                                   childDirection,
                                                   mainAxis,
                                                   mainAxisNodeSize,
                                                   crossAxisNodeSize,
                                                   nodeWidth);
      requirePostLayout |= !positionIsSet;
    }
    ...
    totalOuterFlexBasis +=
        child->layout.computedFlexBasis + YGNodeMarginForAxis(child, mainAxis, nodeWidth);

    // BEGIN_UNITY jonathanma : Check if the child depends on this node dimensions
    if (!isExactMeasurements && !requirePostLayout) {
        requirePostLayout = YGNodeIsStyleLoosePercentage(child, mainAxis, widthMeasureMode, heightMeasureMode);
    }
    // END_UNITY
  }
  ...
  for (; endOfLineIndex < childCount; lineCount++, startOfLineIndex = endOfLineIndex) {
    ...
    for (uint32_t i = startOfLineIndex; i < childCount; i++, endOfLineIndex++) {
      ...
      if (child->style.positionType != YGPositionTypeAbsolute) {
        const float childMarginMainAxis = YGNodeMarginForAxis(child, mainAxis, nodeWidth);
        const float flexBasisWithMaxConstraints =
            fminf(YGResolveValue(&child->style.maxDimensions[dim[mainAxis]], mainAxisNodeSize),
                  child->layout.computedFlexBasis);
        const float flexBasisWithMinAndMaxConstraints =
            fmaxf(YGResolveValue(&child->style.minDimensions[dim[mainAxis]], mainAxisNodeSize),
    ...
    if (!canSkipFlex) {
      ...
      while (currentRelativeChild != NULL) {
        ...
        const float marginMain =
            YGNodeMarginForAxis(currentRelativeChild, mainAxis, nodeWidth);
        const float marginCross =
            YGNodeMarginForAxis(currentRelativeChild, crossAxis, nodeWidth);
      ...
      if (child->style.positionType == YGPositionTypeAbsolute &&
        ...
        if (child->style.positionType == YGPositionTypeRelative) {
          ...
          if (canSkipFlex) {
            ...
            mainDim += betweenMainDim + YGNodeMarginForAxis(child, mainAxis, nodeWidth) +
                       child->layout.computedFlexBasis;
            crossDim = availableInnerCrossDim;
          } else {
            // The main dimension is the sum of all the elements dimension plus the spacing.
            mainDim += betweenMainDim + YGNodeDimWithMargin(child, mainAxis, nodeWidth);

            // The cross dimension is the max of the elements dimension since
            // there can only be one element in that cross dimension.
            crossDim = fmaxf(crossDim, YGNodeDimWithMargin(child, crossAxis, nodeWidth));
          }
        ...
        if (child->style.positionType == YGPositionTypeAbsolute) {
          ...
          if (alignItem == YGAlignStretch &&
            ...
            if (!YGNodeIsStyleDimDefined(child, crossAxis, availableInnerCrossDim)) {
              ...
              float childCrossSize =
                  !YGFloatIsUndefined(child->style.aspectRatio)
                      ? ((YGNodeMarginForAxis(child, crossAxis, nodeWidth) +
                          (isMainAxisRow ? childMainSize / child->style.aspectRatio
                                         : childMainSize * child->style.aspectRatio)))
                      : crossDim;

              childMainSize += YGNodeMarginForAxis(child, mainAxis, nodeWidth);
            ...
            const float remainingCrossDim =
                containerCrossAxis - YGNodeDimWithMargin(child, crossAxis, nodeWidth);
        ...
        if (child->style.positionType == YGPositionTypeRelative) {
          ...
          if (YGNodeAlignItem(node, child) == YGAlignBaseline) {
            ...
            const float descent =
                ...
                YGNodeMarginForAxis(child, YGFlexDirectionColumn, nodeWidth) - ascent;
                ...
                if (!YGNodeIsStyleDimDefined(child, crossAxis, availableInnerCrossDim)) {
                  const float childWidth =
                      isMainAxisRow ? (child->layout.measuredDimensions[YGDimensionWidth] +
                                       YGNodeMarginForAxis(child, mainAxis, nodeWidth))
                                    : lineHeight;
  ...
  // If it require a post layout it's safe to skip absolute children since they do not affect
  // the layout of this node. Absolute children will be laid out in the following post layout.
  if (performLayout && !requirePostLayout) {
  ...
  // BEGIN_UNITY jonathanma
  if (requirePostLayout) {
    // Measured dimensions include border and padding.
    // Need to add the margins because available dimensions include them.
    const float width = node->layout.measuredDimensions[YGDimensionWidth] + marginAxisRow;
    const float height = node->layout.measuredDimensions[YGDimensionHeight] + marginAxisColumn;
    YGLayoutNodeInternal(node,
                         width,
                         height,
                         parentDirection,
                         YGMeasureModeExactly,
...
// BEGIN_UNITY @jonathanma Namespace
namespace
{
    uint32_t gDepth = 0;
...
float YGRoundValueToPixelGrid(const float value,
  ...
  if (YGFloatsEqual(fractial, 0)) {
    ...
    // BEGIN_UNITY @jonathanma Rounding issue with negative values
    ...
    // BEGIN_UNITY @patrickf https://fogbugz.unity3d.com/f/cases/1037940/
    // roundf() rounds half values away from zero. This causes a consistency problem
    // because positive half values are rounded toward the right of the grid
    // whereas negative half values are rounded toward the left of the grid.
    // To fix this, we ensure we only deal with positive values
    // during the rounding process by offseting negative values
    // before rounding.
    float offset = 0.f;
    if (scaledValue < 0.f)
    {
        offset = floorf(scaledValue);
        scaledValue -= offset;
    }
    // END_UNITY
...
bool YGNodeCanUseCachedMeasurement(const YGMeasureMode widthMode,
  ...
  // BEGIN_UNITY @jeanfrancois for https://fogbugz.unity3d.com/f/cases/1105567/
  // We have to remove all the rounding to fix the above case.
  const float effectiveWidth = width;
  const float effectiveHeight = height;
  const float effectiveLastWidth = lastWidth;
  const float effectiveLastHeight = lastHeight;
  // END_UNITY
...
bool YGLayoutNodeInternal(const YGNodeRef node,
  ...
  if (node->measure) {
    ...
    if (YGFloatsEqual(layout->cachedLayout.availableWidth, availableWidth) &&
        ...
        YGFloatsEqual(layout->cachedLayout.parentWidth, parentWidth) &&
        YGFloatsEqual(layout->cachedLayout.parentHeight, parentHeight) &&
    ...
    for (uint32_t i = 0; i < layout->nextCachedMeasurementsIndex; i++) {
      if (YGFloatsEqual(layout->cachedMeasurements[i].availableWidth, availableWidth) &&
          ...
          YGFloatsEqual(layout->cachedMeasurements[i].parentWidth, parentWidth) &&
          YGFloatsEqual(layout->cachedMeasurements[i].parentHeight, parentHeight) &&
  ...
  if (!needToVisitNode && cachedResults != NULL) {
    ...
    if (cachedResults == NULL) {
      ...
      newCacheEntry->parentWidth = parentWidth;
      newCacheEntry->parentHeight = parentHeight;
...
static void YGRoundToPixelGrid(const YGNodeRef node,
  ...
  // BEGIN_UNITY @patrickf https://fogbugz.unity3d.com/f/cases/1037940/
  // Do not round values if layout has not changed, otherwise rounded values could
  // be unstable because of floating point errors when computing the absoluteNode* 
  // values above. Anyway, if layout did not change, values should already be rounded.
  ...
  // END_UNITY
    ...
    // BEGIN_UNITY @jeanfrancois for https://fogbugz.unity3d.com/f/cases/1105567/
    // We have to remove all the rounding to fix the above case.
    // For two elements, with the same layout (position and dimensions) but one without measure and one with measure, the results are different... causing visual artifacts.
    const float roundedAbsoluteNodeLeft = YGRoundValueToPixelGrid(absoluteNodeLeft, pointScaleFactor, false, false);
    const float roundedAbsoluteNodeRight = YGRoundValueToPixelGrid(absoluteNodeRight, pointScaleFactor, false, false);
    const float roundedAbsoluteLeft = YGRoundValueToPixelGrid(absoluteLeft, pointScaleFactor, false, false);

    const float roundedAbsoluteNodeTop = YGRoundValueToPixelGrid(absoluteNodeTop, pointScaleFactor, false, false);
    const float roundedAbsoluteNodeBottom = YGRoundValueToPixelGrid(absoluteNodeBottom, pointScaleFactor, false, false);
    const float roundedAbsoluteTop = YGRoundValueToPixelGrid(absoluteTop, pointScaleFactor, false, false);

    node->layout.position[YGEdgeLeft] = roundedAbsoluteNodeLeft - roundedAbsoluteLeft;
    node->layout.position[YGEdgeTop] = roundedAbsoluteNodeTop - roundedAbsoluteTop;

    node->layout.dimensions[YGDimensionWidth] = roundedAbsoluteNodeRight - roundedAbsoluteNodeLeft;
    node->layout.dimensions[YGDimensionHeight] = roundedAbsoluteNodeBottom - roundedAbsoluteNodeTop;
    // END_UNITY
  // BEGIN_UNITY @patrickf https://fogbugz.unity3d.com/f/cases/1037940/
  }
  // END_UNITY
...
void YGNodeCalculateLayout(const YGNodeRef node,
  ...
  if (YGLayoutNodeInternal(node,
                           ...
                           "initial",
                           node->config)) {
    YGNodeSetPosition(node, node->layout.direction, YGFlexDirectionColumn, parentWidth, parentHeight, parentWidth);
...
static void YGVLog(const YGConfigRef config,
  ...
  // BEGIN_UNITY @jonathanma Fatal logs cause crashes
  // We log them and keep going instead (what's the worse that can happen?!)
